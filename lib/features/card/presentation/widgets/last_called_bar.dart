...
class _LastCalledBarState extends ConsumerState<LastCalledBar> {
  ...
  Widget build(BuildContext context) {
    // We use a listener that triggers only once per socket event
    // Watch the FutureProvider
    final socketAsync = ref.watch(socketProvider(widget.matchId));

    // Once the socket is ready, set up the listener
    socketAsync.whenData((socket) {
      socket.off('eventUpdated');
      socket.on('eventUpdated', (_) {
        ref.invalidate(latestEventProvider(widget.matchId));
      });
    });
    ...
    await showAdOverlay(
      ...
      onAdCompleted: () async {
        ...
        final history = await ref
            ...
            .getEventHistory(widget.matchId);
        // 2. Open Bottom Sheet
